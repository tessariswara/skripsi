...
const Logging: React.FC = () => {
  ...
  useEffect(() => {
    const dataLog = async () => {
      ...
      try {
        ...
        const apiLog = `http://178.128.107.238:5000/api/log/Log/${selectedDevice}?from=${from}&>
        console.log(apiLog);
  ...
  if (selectedDevice && dateFrom && dateTo) {
    try {
      const from = dateFrom.toLocaleDateString();
      const to = dateTo.toLocaleDateString();
      const apiDownloadLog = `http://178.128.107.238:5000/api/log/Log/download/${selectedDevice>
                      ...
                      <div className='page-header log'>
                        <div className='page-control'>
                            <Select
                              ...
                              value={selectedDevice !== '' ? { value: selectedDevice, label: se>
                    ...
                    <div className='chart'>
                       {flagValue ? (
                        <LineChart
                          ...
                          series = {[
                                     {showMark: false, curve: 'natural', data: xData, label: 't>
                                     {showMark: false, curve: 'natural', data: xData2, label: '>
                                   ]}
                        ...
                        <LineChart
                          ...
                          series = {[ {showMark: false, curve: 'natural', data: xData, label: '>
                          xAxis = {[{ scaleType: 'time' ,data: yData }]}
                        />
                         )}
                   </div>
